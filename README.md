...
> [!WARNING]  
Models are generated from fluxcd **2.9.5** using **Cloudcoil 0.7**. Run `uv sync` followed by `make gen-models` to regenerate them from the pinned upstream schema. Cloudcoil infers resource names and normalizes Kubernetes schemas without project-specific patches.
