...
class MatchExpression(BaseModel):
    class Builder(BaseBuilder):
        @property
        def base_class(self) -> Type["MatchExpression"]:
...
class PolicySelector(BaseModel):
    class Builder(BaseBuilder):
        @property
        def base_class(self) -> Type["PolicySelector"]:
        ...
        def match_expressions(
            ...
            value = value_or_callback
            if callable(value_or_callback):
                value = value_or_callback(MatchExpression.list_builder()).build()
    ...
    class Builder(BaseBuilder):
        ...
        def policy_selector(
            ...
            value_or_callback: Union[
                ...
                Callable[[PolicySelector.Builder], PolicySelector.Builder],
            ],
        ) -> Self:
            value = value_or_callback
            if callable(value_or_callback):
                value = value_or_callback(PolicySelector.builder()).build()
...
class ObservedPolicies(BaseModel):
    class Builder(BaseBuilder):
        @property
        def base_class(self) -> Type["ObservedPolicies"]:
...
class ImagePolicy(Resource):
    class Builder(BaseBuilder):
        ...
        def api_version(self, value: Optional[Literal["image.toolkit.fluxcd.io/v1beta2"]]) -> Self:
            return self._set("api_version", value)
